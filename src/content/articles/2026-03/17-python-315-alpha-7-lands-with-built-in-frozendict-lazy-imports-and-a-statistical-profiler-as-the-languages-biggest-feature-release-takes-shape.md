---
title: Python 3.15 Alpha 7 Lands With Built-in frozendict, Lazy Imports, and a Statistical Profiler as the Language's Biggest Feature Release Takes Shape
date: "2026-03-17T12:27:08.331Z"
tags:
  - "Python"
  ...
  - "Performance"
category: News
summary: Python 3.15.0a7 introduces frozendict as a built-in type, explicit lazy imports for faster startup, a statistical sampling profiler, and JIT compiler improvements delivering up to 9% speedups.
sources:
  - "https://docs.python.org/3.15/whatsnew/3.15.html"
  - "https://blog.python.org/2026/03/python-3150-alpha-7/"
  - "https://www.theregister.com/2025/11/04/python_to_embrace_lazy_imports/"
provenance_id: 2026-03/17-python-315-alpha-7-lands-with-built-in-frozendict-lazy-imports-and-a-statistical-profiler-as-the-languages-biggest-feature-release-takes-shape
...
The Python core development team [released Python 3.15.0 alpha 7](https://blog.python.org/2026/03/python-3150-alpha-7/) on March 10, the seventh of eight planned alpha releases before the beta freeze on May 5. The release consolidates a sweeping set of language additions that collectively represent one of Python's most feature-rich development cycles in years, headlined by a built-in immutable dictionary type, a new keyword for deferred module loading, and a production-grade statistical profiler.

## What We Know

### frozendict Arrives as a Built-in Type

The marquee addition is `frozendict`, a new immutable mapping type added to Python's builtins under [PEP 814](https://docs.python.org/3.15/whatsnew/3.15.html), authored by Victor Stinner and Donghee Na. The Python Steering Council accepted the proposal in February, reaching Final status on February 11.
...
Notably, `frozendict` does not inherit from `dict`. It inherits directly from `object` and implements the `collections.abc.Mapping` protocol. According to the [official documentation](https://docs.python.org/3.15/whatsnew/3.15.html), this design choice keeps immutability guarantees clean and prevents accidental mutation through inherited methods. The type supports the union operator (`|`) for merging and preserves insertion order, though comparison is order-independent.
...
Python 3.15 introduces the `lazy` keyword for deferred module loading under [PEP 810](https://docs.python.org/3.15/whatsnew/3.15.html), proposed by Pablo Galindo Salgado. As [The Register reported](https://www.theregister.com/2025/11/04/python_to_embrace_lazy_imports/) when the PEP was approved in November 2025, the feature addresses a longstanding frustration: large applications with deep dependency trees suffer from slow startup because Python must locate, read, compile, and execute all imported module code at load time, even when most of it is never used.
...
PEP 799 introduces a high-frequency, low-overhead statistical sampling profiler and a dedicated `profiling` package to the standard library, according to the [What's New documentation](https://docs.python.org/3.15/whatsnew/3.15.html). The profiler supports sampling rates up to 1,000,000 Hz and can attach to running processes, profile wall time, CPU time, or GIL holding time, and output results in multiple formats including flamegraphs, heatmaps, and a live TUI.
